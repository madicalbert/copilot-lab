...
# Filter the data to exclude betak of -0.9 and sigmaw of .1 using onegambound_3_11.Rdata
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    one_gambound_3_11 %>% 
      ...
      ggplot(aes(x = betak, y = pct_h, fill = alphak)) +
      geom_bar(stat = "identity") +
      labs(title = "One-Gambler Bound",
           x = "betak",
           y = "pct_h",
           fill = "alphak")
  })
